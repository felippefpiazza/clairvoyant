...
function FComma(SS) { var T='', S=String(SS), L=S.length-1, C, j, P = S.indexOf('.')-1
  S = S.replace('.',',');
...
function real_formatado(numero) {
  return 'R$ ' + FComma((Math.round(parseFloat(numero)*Math.pow(10,2))/Math.pow(10,2)).toFixed(2));
}

function dinheiro_formatado(numero) {
  return FComma((Math.round(parseFloat(numero)*Math.pow(10,2))/Math.pow(10,2)).toFixed(2));
}

function dinheiro(v) {
  var d = decimal_comsinal(v);
  var n = d.indexOf('-') >= 0 ? true : false
  
	return (n ? '-' : '') + FComma(decimal(d));
}
